...
function love.draw()
    ...
    -- draw the board with and offset so it's centered on the screen
    drawBoard(128, 16)
    ...
    for y = 1, 8 do
        ...
        for x = 1, 8 do
            ...
            table.insert(tiles[y], {
                ...
                -- coordinates are 0-based, so subtract one before multplying by 32
                x = (x-1) * 32,
                y = (y-1) * 32,

                -- assign a random ID to tile to make it a random tile
                tile = love.math.random(#tileQuads)
            })
        end
    end
