...
function Dungeon:init(player)
    ...
    -- trigger camera translation and adjustment of rooms whenever the player triggers
    -- a shift via a doorway collision, triggered in PlayerWalkState
    ...
    Event.on('shift-up', function()
        self:beginShifting(0, -VIRTUAL_WIDTH)
    end)
...
--[[
    Resets a few variables needed to perform a camera shift and swaps the next and current room.
]]
function Dungeon:finishShifting()
    ...
    -- point to transitioned rom as the new active room, pointing to an empty room next
