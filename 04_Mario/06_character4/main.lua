...
function love.load()
    ...
    love.window.setTitle('character4')
...
function love.keypressed(key)
    ...
    if key == 'space' and charactedDY == 0 then
        charactedDY = JUMP_VELOCITY
...
function love.update(dt)
    ...
    charactedDY = charactedDY + GRAVITY
    characterY = characterY + charactedDY * dt
...
function love.draw()
    ...
    -- we also check for our firection and scale by -1 on the X axis if we're facing left
